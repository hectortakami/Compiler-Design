...
namespace Chimera
{
    ...
    class SemanticAnalyzer
    {
        ...
        public SemanticAnalyzer()
        {
            ...
            procedureTable["AtStr"].symbols["s"] = new SymbolTable.Row(Type.STRING, ProcedureType.PARAM, pos: 0);
            procedureTable["AtStr"].symbols["i"] = new SymbolTable.Row(Type.INT, ProcedureType.PARAM, pos: 1);

            procedureTable["LenStr"] = new ProcedureTable.Row(Type.INT, true);
            procedureTable["LenStr"].symbols["s"] = new SymbolTable.Row(Type.STRING, ProcedureType.PARAM, pos: 0);

            procedureTable["CmpStr"] = new ProcedureTable.Row(Type.INT, true);
            procedureTable["CmpStr"].symbols["s1"] = new SymbolTable.Row(Type.STRING, ProcedureType.PARAM, pos: 0);
            procedureTable["CmpStr"].symbols["s2"] = new SymbolTable.Row(Type.STRING, ProcedureType.PARAM, pos: 1);

            procedureTable["CatStr"] = new ProcedureTable.Row(Type.STRING, true);
            procedureTable["CatStr"].symbols["s1"] = new SymbolTable.Row(Type.STRING, ProcedureType.PARAM, pos: 0);
            procedureTable["CatStr"].symbols["s2"] = new SymbolTable.Row(Type.STRING, ProcedureType.PARAM, pos: 1);

            procedureTable["LenLstInt"] = new ProcedureTable.Row(Type.INT, true);
            procedureTable["LenLstInt"].symbols["loi"] = new SymbolTable.Row(Type.INT_LIST, ProcedureType.PARAM, pos: 0);

            procedureTable["LenLstStr"] = new ProcedureTable.Row(Type.INT, true);
            procedureTable["LenLstStr"].symbols["los"] = new SymbolTable.Row(Type.STRING_LIST, ProcedureType.PARAM, pos: 0);

            procedureTable["LenLstBool"] = new ProcedureTable.Row(Type.INT, true);
            procedureTable["LenLstBool"].symbols["lob"] = new SymbolTable.Row(Type.BOOL_LIST, ProcedureType.PARAM, pos: 0);

            procedureTable["NewLstInt"] = new ProcedureTable.Row(Type.INT_LIST, true);
            procedureTable["NewLstInt"].symbols["size"] = new SymbolTable.Row(Type.INT, ProcedureType.PARAM, pos: 0);

            procedureTable["NewLstStr"] = new ProcedureTable.Row(Type.STRING_LIST, true);
            procedureTable["NewLstStr"].symbols["size"] = new SymbolTable.Row(Type.INT, ProcedureType.PARAM, pos: 0);

            procedureTable["NewLstBool"] = new ProcedureTable.Row(Type.BOOL_LIST, true);
            procedureTable["NewLstBool"].symbols["size"] = new SymbolTable.Row(Type.INT, ProcedureType.PARAM, pos: 0);

            procedureTable["IntToStr"] = new ProcedureTable.Row(Type.STRING, true);
            procedureTable["IntToStr"].symbols["i"] = new SymbolTable.Row(Type.INT, ProcedureType.PARAM, pos: 0);

            procedureTable["StrToInt"] = new ProcedureTable.Row(Type.INT, true);
            procedureTable["StrToInt"].symbols["s"] = new SymbolTable.Row(Type.STRING, ProcedureType.PARAM, pos: 0);
        }
        ...
        public Type Visit(ConstantDeclarationNode node)
        {
            ...
            AddSymbolToScope(varName, type, ProcedureType.CONST, value: GetValueFromNode(node[0]));
            return type;
        }
        ...
        public Type Visit(VariableDeclarationNode node)
        {
            foreach (var typeNode in node)
            {
                ...
                foreach (var idNode in typeNode)
                {
                    ...
                    {
                        AddSymbolToScope(varName, type, ProcedureType.VAR, value: type.DefaultValue());
                    }
                }
            }
            return Type.VOID;
        ...
        public Type Visit(AssignmentNode node)
        {
            ...
            if (node[0] is IdentifierNode && GetSymbol(node[0].AnchorToken.Lexeme).kind == ProcedureType.CONST)
            {
        ...
        public Type Visit(ForStatementNode node)
        {
            ...
            AddSymbolToScope($"__{node[0].AnchorToken.Lexeme}_index", Type.INT, ProcedureType.VAR, value: 0);
            var lastInLoopOrFor = inLoopOrFor;
            inLoopOrFor = true;
        ...
        public Type Visit(ParameterDeclarationNode node)
        {
            foreach (var typeNode in node)
            {
                ...
                foreach (var idNode in typeNode)
                {
                    ...
                    {
                        AddSymbolToScope(varName, type, ProcedureType.PARAM, pos: pos++, value: type.DefaultValue());
                    }
                }
            }
            return Type.VOID;
        ...
        void AddSymbolToScope(string key, Type type, ProcedureType kind, dynamic value = null, int pos = -1)
        {
            SymbolTable table;
