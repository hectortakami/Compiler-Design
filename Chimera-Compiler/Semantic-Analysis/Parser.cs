        ...
        public Node UnaryExpression()
        {
            switch (CurrentToken)
            {
                case TokenCategory.COMP:
                    var compNode = new CompNode()
                    {
                        AnchorToken = Expect(TokenCategory.COMP)
                    };
                    compNode.Add(UnaryExpression());
                    return compNode;
